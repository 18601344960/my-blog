...
public class IndexController extends BaseController {
	...
	public String post(Model model, @PathVariable(value = "articleUrl") String articleUrl, HttpServletRequest request) {
		...
		if (!checkRepeatIp(request, articleCustom.getId())) {
			articleCustom.setArticleViews(updateArticleViews(articleCustom.getId(), articleCustom.getArticleViews()));
		}
	 ...
	 * @param views
	 */
	public Long updateArticleViews(Integer id, Long views) {
		if (views == null) {
			views = 0L;
			return views;
		}
		...
		return article.getArticleViews();
	}
